...
namespace Aura {
	...
	public abstract class RecordManager {
        ...
        internal abstract void SetupManager(string collectionName, string connectionName);

        internal List<RecordManagerIndex> Indexes { get; set; }

        internal class RecordManagerIndex
        {
            public IMongoIndexKeys Keys { get; set; }
        ...
        /// <summary>
        /// The name of the MongoDB connection to use
        /// </summary>
        ...
        /// <summary>
        /// Returns an instance of MongoCollection that isn't typed. Useful for operating with non-deserialized instances of the documents, for instance, during schema migration
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Removes the specified property from all documents in the collection. Useful during schema migration.
        /// </summary>
	...
	public class RecordManager<T> : RecordManager, IRecordManager<T> where T : class {
		...
		public RecordManager(string collectionName = null, string connectionName = null) {
            SetupManager(collectionName, connectionName);
		}
		...
		public virtual T GetById(object id) {
			logger.Debug(String.Format("GetById({0})", id));
			if (id == null) {
		...
		protected internal long Count(IMongoQuery criteria = null) {
			logger.Debug(String.Format("Count({0})", criteria));
		...
		protected internal virtual IEnumerable<T> Find(IMongoQuery criteria, IMongoSortBy sort = null, int? skip = null, int? limit = null) {
			logger.Debug(String.Format("Find({0},{1},{2},{3})", criteria, sort, skip, limit));
		...
		protected internal virtual T FindSingle(IMongoQuery criteria) {
			logger.Debug(String.Format("FindSingle({0})", criteria));
		...
		protected internal virtual T FindOneAndModify(IMongoQuery criteria, IMongoUpdate update, IMongoSortBy sortBy = null, bool returnModifiedVersion = true) {
			logger.Debug(String.Format("FindAndModify({0},{1},{2},{3})", criteria, update, sortBy, returnModifiedVersion));
		...
		protected internal virtual T FindOneAndRemove(IMongoQuery criteria, IMongoSortBy sortBy = null) {
			logger.Debug(String.Format("FindOneAndRemove({0},{1})", criteria, sortBy));
		...
		protected internal IEnumerable<T> EnumerateAndModify(IMongoQuery criteria, IMongoUpdate update, IMongoSortBy sortBy = null, bool returnModifiedVersion = true) {
			logger.Debug(String.Format("EnumerateAndModify({0},{1},{2},{3})", criteria, update, sortBy, returnModifiedVersion));
			T result;
		...
		protected internal IEnumerable<T> EnumerateAndRemove(IMongoQuery criteria, IMongoSortBy sortBy = null) {
			logger.Debug(String.Format("EnumerateAndRemove({0},{1})", criteria, sortBy));
			T result;
		...
		protected internal virtual long UpdateMany(IMongoQuery criteria, UpdateBuilder update, bool asUpsert = false) {
			logger.Debug(String.Format("UpdateMany({0},{1},{2})", criteria, update, asUpsert));
		...
		protected internal virtual long DeleteMany(IMongoQuery criteria) {
			logger.Debug(String.Format("DeleteMany({0})", criteria));
			if (criteria == null) {
		...
		protected internal virtual void EnsureIndex(IMongoIndexKeys keys, IMongoIndexOptions options) {
            ...
            if (Indexes == null)
            {
                Indexes = new List<RecordManagerIndex>();
            }
		...
		/// Convenience method to create a Query criteria based upon the ID field.
		/// </summary>
        /// <param name="IdList">A list of values for Ids for which to query</param>
		/// <returns>A MongoQuery criteria for the equality of the specified Id.</returns>
        ...
        protected IEnumerable<BsonDocument> Aggregate(params BsonDocument[] operations)
        {
            ...
            return results.ResultDocuments;
		}
        ...
        {
            var results = collection.Aggregate(operations);
            return results.ResultDocuments.Select(x => BsonSerializer.Deserialize<TReturnType>(x));
		}
